   ...
   try {
      await Promise.all([
         ...
         global.db.createTable('uploader', [
            ...
            { name: 'folder_id', type: 'INTEGER DEFAULT NULL' },
      ...
      // Migration for existing uploader table
      const uploaderColumns = await global.db.getAll('uploader')
      const hasUserId = uploaderColumns.some(col => col.name === 'user_id')
      if (!hasUserId) {
         await global.db.run('ALTER TABLE uploader ADD COLUMN user_id INTEGER DEFAULT NULL')
         await global.db.run('ALTER TABLE uploader ADD COLUMN folder_id INTEGER DEFAULT NULL')
      }
